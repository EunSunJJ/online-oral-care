			...
			<div class="qpost-list-box">
				...
				<div class="qpost-search">
					<form name="searchFrm" method="get" action="/qpost/list-view">
						...
						<button id="searchBtn" class="button-qpost-search" type="submit"  value="${pagination.keyword}">검색</button>
					</form>
				</div>
					...
					<div id="paginationBox">
						<ul>
							...
							<c:if test="${pagination.keyword != null and pagination.searchType != null}">
								...
								<!-- 맨 처음 페이지로 -->
								<li class="pageInfo"><a class="paging" href="/qpost/list-view?searchType=${pagination.searchType}&keyword=${pagination.keyword}&page=1">처음으로</a></li>
				                
				                <!-- 이전페이지 버튼 -->
				                <c:if test="${pagination.prev}">
				                    <li class="pageInfo"><a class="paging" 
				                    href="/qpost/list-view?searchType=${pagination.searchType}&keyword=${pagination.keyword}&page=${pagination.startPage - 1}">이전</a></li>
				                </c:if>
								
								<!-- 페이지 리스트 -->
								<c:forEach var="num" begin="${pagination.startPage}" end="${pagination.endPage}">
									<li class="pageInfo"><a class="paging" 
									href ="/qpost/list-view?searchType=${pagination.searchType}&keyword=${pagination.keyword}&page=${num}"> ${num}</a></li>
								</c:forEach>
								
	               				<!-- 다음페이지 버튼 -->
	               				 <c:if test="${pagination.next}">
	               				 	<li class="pageInfo"><a class="paging" 
	               				 	href="/qpost/list-view?searchType=${pagination.searchType}&keyword=${pagination.keyword}&page=${pagination.endPage + 1}">다음</a></li>
	                			</c:if>  							
							</c:if> 
						</ul>	
					</div>
